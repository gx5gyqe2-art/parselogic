...
| ファイル | 役割 |
| --- | --- |
| `fetch_html.py` | **公式サイトからHTMLを直接取得**して差分取り込みまで実行（iPhone/Pythonista推奨） |
...
### ★ 推奨（iPhone/Pythonista 3 含む）: 公式サイトから直接取得

ページを「テキスト保存」すると HTMLタグ・画像URL・属性が失われ、パーサが**1枚も読めない**
（「差分なし」になる原因）。手動保存に頼らず、公式サイトから本物のHTMLを直接取得する:

```
# fetch_html.py を開く
1) そのまま Run → 取り込み可能な「シリーズ名 → コード」一覧が表示される
2) 新弾のコードを SERIES に書いて再度 Run
   例: SERIES = ["550031","550032","550033","550034","550035","550036"]  # ST-31〜36
       SERIES = ["550116"]                                               # OP-16
   取得 → 既存マスターへ差分マージ → 新カード画像の取得まで自動
```

- 取得HTMLは `series_<コード>.html` として保存（`.gitignore` 済み）
- 全パスは「スクリプトの場所」基準で読み書きするため、Pythonista で作業ディレクトリが
  変わっても正しく動く（cwd非依存）
- 画像取得を止めたいときは `NO_IMAGE = True`

参考コード（2026-07時点）: OP-16=`550116` / ST-31〜36=`550031`〜`550036` /
プロモ=`550901`。最新の対応表は `fetch_html.py` を空 SERIES で Run すれば表示される。
